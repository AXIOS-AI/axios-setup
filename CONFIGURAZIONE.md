...
**Prima di tutto**, verifica che siano installate e attive le estensioni, **spiegami a cosa serve ciascuna e chiedimi se le voglio installare** (elenco reale dei pacchetti attivi su questa macchina, da `~/.pi/agent/settings.json`):
...
**Poi** leggi il database nella cartella indicata e analizza la sua struttura (tabelle, campi, relazioni). Se non riconosci il tipo di database, chiedi all'utente se lo conosce; altrimenti cerca online le informazioni necessarie per capirlo e procedi **solo se hai conferma di averlo identificato**.

**Configura** il motore NL-query scelto, collegato a quel database.

**Crea** per me le regole di comportamento dell'agente (stile delle risposte, sicurezza, limiti, formato). Applica queste regole operative:
...
**Crea** per questo progetto un file `PROJECT.md` con le regole del progetto (struttura cartelle, convenzioni, cose da non toccare).

**Crea** un alias con il nome `axios` (conformità AI Act: io resto deployer, il ruolo di provider resta allo sviluppatore).

**Fammi** una domanda di prova sul database per verificare che il motore NL-query funzioni.
